...
public class KeycloakContainer extends GenericContainer<KeycloakContainer> {
    ...
    private static final String KEYCLOAK_VERSION = "20.0.3-0";
    ...
    protected void configure() {
        ...
        setWaitStrategy(Wait.forHttp(contextPath).forPort(KEYCLOAK_PORT_HTTP).withStartupTimeout(startupTimeout));
    ...
    protected void createKeycloakExtensionDeployment(String deploymentLocation, String extensionName, String extensionClassFolder) {
        ...
        if (new File(classesLocation).exists()) {
            ...
            try {
                ...
                file.deleteOnExit();
                ShrinkWrap.create(JavaArchive.class, extensionName).as(ExplodedImporter.class)
                        .importDirectory(classesLocation).as(ZipExporter.class).exportTo(file, true);
                withCopyFileToContainer(MountableFile.forHostPath(file.getAbsolutePath()),
                        deploymentLocation + "/" + extensionName);
            } catch (IOException e) {
    ...
    protected String resolveExtensionClassLocation(String extensionClassFolder) {
        return Paths.get(MountableFile.forClasspathResource(".").getResolvedPath()).getParent().getParent().resolve(extensionClassFolder).toString();
    }
    ...
    public Keycloak getKeycloakAdminClient() {
        if (useTls) {
            return Keycloak.getInstance(getAuthServerUrl(), MASTER_REALM,
                    getAdminUsername(), getAdminPassword(), ADMIN_CLI_CLIENT, buildSslContext());
        } else {
            return Keycloak.getInstance(getAuthServerUrl(), MASTER_REALM,
                    getAdminUsername(), getAdminPassword(), ADMIN_CLI_CLIENT);
        }
    }
        ...
        try {
            ...
            if (this.tlsKeystoreFilename != null) {
                ...
                X509Certificate certificate = (X509Certificate) certificateFactory
                        .generateCertificate(loadResourceAsStream(this.tlsCertificateFilename));
    ...
    public String getAuthServerUrl() {
        return String.format("http%s://%s:%s%s", useTls ? "s" : "",
                getContainerIpAddress(), useTls ? getHttpsPort() : getHttpPort(), getContextPath());
    }
